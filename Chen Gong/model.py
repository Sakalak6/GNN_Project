...
class GraLSP(nn.Module):
    def __init__(self,parser,
        ...
        self.num_heads = num_heads  # set the number of heads
        self.weight_attention = nn.ParameterList([
            nn.Parameter(torch.FloatTensor(self.walk_dim, 1)) for _ in range(self.num_heads)
        ])
        self.bias_attention = nn.ParameterList([
            nn.Parameter(torch.FloatTensor(1)) for _ in range(self.num_heads)
        ])

        # Initialize attention parameters for each head
        for weight in self.weight_attention:
            nn.init.xavier_normal_(weight)
        for bias in self.bias_attention:
            nn.init.constant_(bias, 0.01)

        # Additional linear transformation if concatenating heads
        self.linear_multi_head = nn.Linear(self.num_heads * self.walk_dim, self.walk_dim)
    ...
    def build_model(self):
        ...
        #############  Single attention parameters
        # self.weight_attention = nn.ParameterList([
        #     nn.Parameter(torch.FloatTensor(self.walk_dim, 1)),
        #     nn.Parameter(torch.FloatTensor(self.walk_dim, 1))
        # ])
        # self.bias_attention = nn.ParameterList([
        #     nn.Parameter(torch.FloatTensor(1)),
        #     nn.Parameter(torch.FloatTensor(1))
        # ])
    ...
    def _aggregate(self, sample_nodes, sample_paths, input_size):
        ...
        for layer in range(2):
            ...
            for hop in range(2-layer): 
                ...
                ##############  Apply attention #################
                # Attention weights
                # weight_attention = self.weight_attention[layer]
                # bias_attention = self.bias_attention[layer]
                # attention_weights = F.softmax(torch.matmul(path_vecs, weight_attention) + bias_attention, dim=1) # attention
  
                # # weigh_path: [30,32]
                # channel_amplifier = torch.sigmoid(torch.matmul(path_vecs, weight_path) + bias_path)
                # neigh_mean = torch.mean(channel_amplifier * neigh_vecs * attention_weights, axis=1).type(torch.float) 
                # # print("neigh_mean", neigh_mean.shape)
                ...
                for head in range(self.num_heads):
                    weight_attention = self.weight_attention[head]
                    bias_attention = self.bias_attention[head]

                    # Compute attention for this head
                    attention_weights = F.softmax(torch.matmul(path_vecs, weight_attention) + bias_attention, dim=1)
                    neigh_mean = torch.mean(channel_amplifier * neigh_vecs * attention_weights, axis=1).type(torch.float)
                    multi_head_outputs.append(neigh_mean)
                # print("multi_head_outputs", len(multi_head_outputs))
                # Combine outputs from all heads
                # combined_heads = torch.cat(multi_head_outputs, dim=-1)  # Concatenate
                # combined_heads = self.linear_multi_head(combined_heads).type(torch.float)

                # combined_heads = torch.mean(torch.stack(multi_head_outputs), dim=0)  # Average
                combined_heads = torch.sum(torch.stack(multi_head_outputs), dim=0).type(torch.float)  # Summation
                # print("combined_heads", combined_heads.shape)
                # Additional linear transformation if concatenating


                from_neighs = combined_heads @ weight_neigh
                from_self = hidden_nodes[hop].type(torch.float) @ weight_self
                ...
                ############## Test #################
                # print("channel_amplifier", channel_amplifier.shape)
                # print("attention_weights", attention_weights.shape)
                # print("neigh_vecs", neigh_vecs.shape)
                # print("channel*vecs", (channel_amplifier * neigh_vecs).shape)
                # print("attention_wights.unsqueeze(-1)", attention_weights.unsqueeze(-1).shape)
                # print("produc", channel_amplifier * neigh_vecs * attention_weights.unsqueeze(-1).shape)
